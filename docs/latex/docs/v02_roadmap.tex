...
real: - \textbf{Lightning Network Real (§18.1 y §34)}: Pagar facturas
automáticamente desde un nodo Lightning integrado. - \textbf{Transmisión
On-chain Real (§34)}: Construcción de PSBTs (Partially Signed Bitcoin
Transactions), firma de transacciones y \emph{broadcasting} a la
mempool. - \textbf{Implementación Real de Ark (§18.3 y §34)}: Conectarse
a un ASP (Ark Service Provider) real para transferir VTXOs. -
...
múltiples receptores. - \textbf{Custodial Escrow (§34)}: Depósito de
garantía en custodia temporal (marcado como ``Do not build yet'').
...
\textbf{Políticas de Recuperación de Llaves (Multisig)}: Para mitigar la
pérdida de la llave de identidad. - \textbf{Invitaciones Firmadas y
Verificación de Dominio (§27)}: Para mitigar ataques de enlaces de
invitación maliciosos. - \textbf{Validación de Pruebas y Reputación de
Servidores Ark (§27)}: Para proteger a los usuarios de ASPs maliciosos.
