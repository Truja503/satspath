...
The Rust quote response currently exposes
\texttt{identifier\_verified:\ false} for the default resolver chain
because resolver provenance is not yet carried through the
\texttt{ProfileResolver} trait. Future platform or provenance-aware
resolvers may set it to \texttt{true} only after binding the canonical
identifier to the current signed profile identity.
