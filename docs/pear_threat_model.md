...
| **Replay Attacks**                       | The `SignedPaymentProfile` includes `expires_at` and `sequence` fields. `Registry::update_profile_for` strictly enforces monotonically increasing sequence numbers. Note that stateless remote/P2P Rust resolvers validate signatures and expiry without tracking sequence history, so time-bound expiry is the primary mitigation against stale replayed profiles during remote discovery. | Rust Core |
