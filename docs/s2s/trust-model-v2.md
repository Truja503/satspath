...
| **Hosted Provider Administrator** | Manages server infrastructure for hosted sub-identities (`user@provider.com`) | Infrastructure root / DB admin | Can censor or terminate account, but *cannot* forge transitions for existing user-held keys | Explicit "Hosted" trust rating displayed in UI; portable identity export |
