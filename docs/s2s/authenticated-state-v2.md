...
## Overview
An append-only Merkle log efficiently proves that an event exists in history, but it cannot efficiently prove non-inclusion (i.e. that an identifier is NOT registered) or that an event is the *latest* state. To prevent malicious log operators from selectively hiding identifiers via omission, SatsPath utilizes a dual authenticated structure.
