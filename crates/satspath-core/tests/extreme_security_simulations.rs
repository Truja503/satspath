...
async fn test_attack_memory_exhaustion_dos() {
    ...
    // VALIDATION
    assert!(result.is_err(), "SECURITY FAILURE: Node attempted to parse a 5MB payload!");
    let err_msg = result.unwrap_err().to_string();
    assert!(err_msg.contains("DoS protection"), "Node should explicitly reject via DoS protection. Got: {}", err_msg);
    println!("🛡️ DEFENSE SUCCESS: Memory exhaustion avoided! Download forcefully aborted. Reason: {}", err_msg);
}
...
fn test_attack_pqc_downgrade() {
    ...
    let received_profile: satspath_core::profile::SignedPaymentProfile = serde_json::from_str(&corrupted_json).unwrap();
    
    // VALIDATION
    let verification_result = verify_signed_profile(&received_profile).unwrap_or(false);
    
    assert!(!verification_result, "SECURITY FAILURE: The node accepted a downgraded PQC profile!");
...
async fn test_attack_dns_spoofing() {
    ...
    println!("⚔️ ATTACK 11: Malicious Wi-Fi attempts to poison DNS cache and return fake TXT records...");
    
    // VALIDATION
    // We statically verify that the resolver strictly enforces DNSSEC
    assert!(resolver.dnssec_required(), "SECURITY FAILURE: DNSSEC is disabled! The protocol is vulnerable to spoofing.");
