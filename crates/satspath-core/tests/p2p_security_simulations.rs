...
fn test_attack_p2p_dht_scraping_privacy() {
    ...
    assert_ne!(
        public_topic, alias,
        "SECURITY FAILURE: The plaintext alias leaked into the P2P topic!"
    );
...
fn test_attack_p2p_in_transit_corruption() {
    ...
    let profile = create_testnet_profile(
        "chelo@testnet",
        &alice_pubkey_hex,
        "testnet_node@lightning.dev",
    );
    ...
    let corrupted_payload =
        network_payload.replace("testnet_node@lightning.dev", "hacker_testnet@evil.node");

    // Node receives the corrupted payload and attempts to parse and verify it
    let received_profile: Result<satspath_core::profile::SignedPaymentProfile, _> =
        serde_json::from_str(&corrupted_payload);

    // VALIDATION
    assert!(
        received_profile.is_ok(),
        "Payload should still parse as JSON."
    );
    ...
    assert!(
        !verification_result,
        "SECURITY FAILURE: The receiving node accepted a payload corrupted in-transit!"
    );
