...
use satspath_core::rotation::{rotate_identity_key, KeyRotation};
...
fn test_attack_payload_tampering() {
    ...
    let alice_profile = create_test_profile("alice@satspath.dev", &alice_pubkey_hex, "alice_real@lightning.node");
    let mut signed_profile = sign_profile(alice_profile, &alice_keys.secret_key).expect("Failed to sign profile");
    
    // Validate that Alice's profile is initially valid
    assert!(verify_signed_profile(&signed_profile).expect("Verification failed"), "Alice's original profile should be valid");
    ...
    if let Some(PaymentMethod::Lightning { ref mut lightning_address, .. }) = signed_profile.profile.methods.get_mut(0) {
    ...
    assert!(!is_valid, "SECURITY FAILURE: The tampered profile was accepted as valid!");
...
fn test_attack_unauthorized_key_rotation() {
    ...
    let bob_profile = create_test_profile("bob@satspath.dev", &bob_pubkey_hex, "bob@lightning.node");
    let signed_bob_profile = sign_profile(bob_profile.clone(), &bob_keys.secret_key).expect("Failed to sign profile");
    ...
    let signed_hijacked_profile = sign_profile(hijacked_profile, &hacker_keys.secret_key).expect("Failed to sign hijacked profile");
    ...
    let is_rotation_valid = satspath_core::rotation::is_rotation_valid(&signed_hijacked_profile).unwrap_or(false);

    assert!(!is_rotation_valid, "SECURITY FAILURE: The unauthorized key rotation was accepted!");
