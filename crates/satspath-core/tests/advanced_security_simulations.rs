use satspath_core::crypto::{generate_identity_keypair, check_profile_expiry};
...
fn test_attack_ssrf_cloud_metadata() {
    ...
    // VALIDATION
    assert!(aws_result.is_err(), "SECURITY FAILURE: SSRF validation allowed connection to AWS Metadata endpoint!");
    assert!(ssh_result.is_err(), "SECURITY FAILURE: SSRF validation allowed connection to localhost/SSH!");
    assert!(private_result.is_err(), "SECURITY FAILURE: SSRF validation allowed connection to private IP space!");
    
    println!("🛡️ DEFENSE SUCCESS: Network firewall explicitly blocked all internal/loopback fetches.");
}
...
fn test_attack_replay_expired_profile() {
    ...
    println!("⚔️ ATTACK 7: Attacker intercepts and replays the 6-month-old zombie profile today...");
    ...
    assert!(expiry_check.is_err(), "SECURITY FAILURE: Replayed expired profile was accepted as valid!");
    
    let err_msg = expiry_check.unwrap_err().to_string();
    println!("🛡️ DEFENSE SUCCESS: Profile strictly rejected due to timestamp expiration. Reason: {}", err_msg);
}
