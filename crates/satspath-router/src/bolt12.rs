...
pub fn parse_bolt12_invoice_request(request: &str) -> Result<Bolt12InvoiceRequest, anyhow::Error> {
    ...
    if !request.starts_with("lnr1") {
        return Err(anyhow!("Invalid BOLT12 invoice request: must start with 'lnr1'"));
    }
...
pub fn validate_bolt12_invoice(invoice: &Bolt12Invoice, expected_amount_msats: u64) -> Result<(), anyhow::Error> {
