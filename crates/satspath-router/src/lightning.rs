...
/// Resolve a Lightning Address (user@domain) to the LNURL-pay metadata endpoint.
///
...
pub async fn fetch_lnurl_metadata(lightning_address: &str) -> anyhow::Result<LnurlPayMetadata> {
    ...
    if parts.len() != 2 {
        anyhow::bail!("invalid Lightning Address format: {}", lightning_address);
    }
    let user = parts[0];
    let domain = parts[1];
...
/// Step 2: call the LNURL-pay callback to get a real BOLT11 invoice.
pub async fn fetch_invoice(
    ...
    if amount_msats < meta.min_sendable {
        anyhow::bail!(
            "amount {} sats ({} msats) is below minimum {} msats",
            amount_sats,
            amount_msats,
            meta.min_sendable
        );
