...
fn test_attack_fee_oracle_manipulation() {
    ...
    let normal_fee = FeeEstimate { fastest_fee: 10, half_hour_fee: 10, hour_fee: 10, economy_fee: 5, minimum_fee: 1 };
    ...
    let malicious_fee = FeeEstimate { fastest_fee: 1000, half_hour_fee: 1000, hour_fee: 1000, economy_fee: 1000, minimum_fee: 1000 };
    
    // VALIDATION
    let attack_decision = select_priority_route(50_000, &malicious_fee, &methods, true).unwrap();
    
    assert_ne!(attack_decision.rail, "onchain", "SECURITY FAILURE: Router attempted on-chain transaction during massive fee spike!");
    assert_eq!(attack_decision.rail, "lightning", "Router should fallback to Lightning.");
    println!("🛡️ DEFENSE SUCCESS: Router automatically abandoned On-chain due to high fees. Reason: {}", attack_decision.reason);
}
...
fn test_attack_routing_blackhole() {
    ...
    let malicious_fee = FeeEstimate { fastest_fee: 200, half_hour_fee: 200, hour_fee: 200, economy_fee: 200, minimum_fee: 200 };
    let routing_ok = false; // Attacker blocks Lightning path
    
    // VALIDATION
    let attack_decision = select_priority_route(50_000, &malicious_fee, &methods, routing_ok).unwrap();
    
    assert_ne!(attack_decision.rail, "onchain", "SECURITY FAILURE: Fell into high fee trap.");
    assert_ne!(attack_decision.rail, "lightning", "SECURITY FAILURE: Attempted censored Lightning route.");
    assert_eq!(attack_decision.rail, "ark", "Router should fallback to Ark layer 3.");
    println!("🛡️ DEFENSE SUCCESS: Router safely fell back to Ark (L3). Reason: {}", attack_decision.reason);
}
...
fn test_attack_extreme_value_liquidity() {
    ...
    let malicious_fee = FeeEstimate { fastest_fee: 500, half_hour_fee: 500, hour_fee: 500, economy_fee: 500, minimum_fee: 500 };
    
    // VALIDATION
    let attack_decision = select_priority_route(massive_payment_sats, &malicious_fee, &methods, true).unwrap();
    
    assert_ne!(attack_decision.rail, "lightning", "SECURITY FAILURE: Router attempted to send a massive amount via Lightning!");
    ...
    println!("🛡️ DEFENSE SUCCESS: Router blocked massive L2 payment to protect liquidity. Reason: {}", attack_decision.reason);
}
