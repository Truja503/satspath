...
export default function SendForm() {
  ...
  useEffect(() => {
    ...
    const parseRecipient = async () => {
      ...
      // --- SATSPATH INTERCEPTION ---
      // If recipient looks like an email alias (e.g. user@domain.com)
      if (recipient.includes('@') && recipient.includes('.')) {
        try {
          const satoshisToRoute = sendInfo.satoshis ?? 0
          if (satoshisToRoute > 0) {
             ...
             if (route) {
               ...
               if (route.methodType === 'lightning') {
                  setSendInfo({ ...sendInfo, invoice: route.payload, satoshis: satoshisToRoute, arkAddress: undefined, address: undefined })
               } else if (route.methodType === 'ark') {
                  setSendInfo({ ...sendInfo, arkAddress: route.payload, invoice: undefined, address: undefined, satoshis: satoshisToRoute })
               } else if (route.methodType === 'onchain') {
                  setSendInfo({ ...sendInfo, address: route.payload, invoice: undefined, arkAddress: undefined, satoshis: satoshisToRoute })
               }
               return
             }
          }
        } catch (e) {
          console.error('SatsPath error', e)
        }
      }
      setSatspathRoute(null)
      // -----------------------------
    ...
    parseRecipient()
  }, [recipient, isAssetSend, readyToParse, sendInfo.satoshis])
