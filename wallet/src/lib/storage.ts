...
export const saveSatsPathProfileToStorage = (data: SatsPathLocalData): void => {
  setStorageItem('satspath_profile', JSON.stringify(data))
}

export const readSatsPathProfileFromStorage = (): SatsPathLocalData | undefined => {
  return getStorageItem('satspath_profile', undefined, (val) => JSON.parse(val))
}
